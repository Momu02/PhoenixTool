...
import "android.view.*"
import "page.forum_layout.chatPrivate"
import "cjson"
import "com.androlua.*"
import "mods.muk"
...
activity.setContentView(loadlayout(chatPrivate))
activity.setRequestedOrientation(1)
title.getPaint().setFakeBoldText(true)
exit.getPaint().setFakeBoldText(true)

url,admin,user,password,key,isEncrypt,secret,toUser=...
...
function loadList(num)
  ...
  --声明接口
  local api=url.."main/api/chat/chat_private.php"
  --请求体
  local body={
    ["admin"]=admin,
    ["user"]=user,
...
function loadNew()
  ...
  --声明接口
  local api=url.."main/api/chat/chat_private_new.php"
  --请求体
  local body={
    ["admin"]=admin,
    ["user"]=user,
...
list.onItemClick=function(parent,v,pos,id)
  ...
  .setPositiveButton("删除",{onClick=function(v)
      local api=url.."main/api/chat/chat_private_delete.php"
      local body={
        ["admin"]=admin,
        ["user"]=user,
        ["password"]=password,
...
sendButton.onClick=function ()
  ...
  if sendContent!="" then
    --发送请求
    local api=url.."main/api/chat/chat_private_send.php"
    local body={
      ["admin"]=admin,
      ["user"]=user,
      ["password"]=password,
